...
int main() {
    ...
    for (int i = 0; i < batch_size; i++) {
        std::copy(origin_test_images[i].begin(), origin_test_images[i].end(),
                  std::back_inserter(x_test_images));
//        x_test_images.insert(x_test_images.end(), origin_test_images[i].begin(),
//                             origin_test_images[i].begin() + features);
    }
...
//    }
    ...
    std::cout << "xin.size: " << xIn.size() << " conv1_weight:" << c1wIn.size() << " conv1_bias: " << c1bIn.size() <<" conv2_weight: " << c2wIn.size()
    << " conv2_bias: " << c2bIn.size() << " fc1_weight: " << f1wIn.size() << " fc1_bias: " << f1bIn.size() << " fc2_weight:" << f2wIn.size()
