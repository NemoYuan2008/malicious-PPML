...
std::shared_ptr<FakeGate<ShrType, N>> ResBlock(FakeCircuit<Spdz2kShare64, 2> &circuit, std::shared_ptr<FakeGate<ShrType, N>> xIn){
...
//    out = circuit.add(out,xIn);
    std::shared_ptr<FakeGate<Spdz2kShare64, 2>> shortcut = circuit.conv2DTrunc(xIn,kernel3,conv3_op);
